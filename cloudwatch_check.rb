...
module AWS
  module ELB
    class Base < AWS::Base
      ...
      #Fix bug in amazon-ec2 gem
      def describe_load_balancers( options = {} )
        options = { :load_balancer_names => [] }.merge(options)
        params = pathlist("LoadBalancerNames.member", options[:load_balancer_names])
  ...
  module Cloudwatch
    class Base < AWS::Base
      ...
      #NB: MetricName replaces MeasureName which was used prior 2010-08-01 API version
...
begin
  ...
  if use_rsa
    ...
    cipher.decrypt
    cipher.key = OpenSSL::Digest::SHA256.digest(File.read(key_file))
...
begin
  if namespace.eql?(AWS_NAMESPACE_EC2)
    ...
    #RDS
    instance = aws_api.describe_db_instances(:db_instance_identifier => instance_id)
    ...
    if instance.DescribeLoadBalancersResult.LoadBalancerDescriptions.nil? || instance.DescribeLoadBalancersResult.LoadBalancerDescriptions.empty?
      ...
      #cloudwatch_enabled = EC2_STATE_ENABLED
      cloudwatch_enabled = EC2_STATE_DISABLING
    end
  end
rescue Exception => e
...
if (cloudwatch_metrics_stats.nil? || cloudwatch_metrics_stats.empty?)
  ...
  if /Sum/i =~ stat
    sum = sprintf( "%.2f", cloudwatch_metrics_stats.GetMetricStatisticsResult.Datapoints.member[0].Sum )
    cloudwatch_svc_output += ", Sum: #{sum}"
    cloudwatch_svc_perfdata += "metric_sum=#{sum} "
  end
  if /SampleCount/i =~ stat
    sample_count = sprintf( "%.2f", cloudwatch_metrics_stats.GetMetricStatisticsResult.Datapoints.member[0].SampleCount )
